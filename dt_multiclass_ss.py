...
def parse_arguments():
    ...
    args.output_folder = check_dir(os.path.join(args.output_root, 'dt_multiclass_seg', args.exp_name))
...
def main(args):
    ...
    pretrained_encoder.load_state_dict(torch.load(args.weights_init_encoder, map_location=device)['model'], strict=False)
    
    model = Segmentator(6, pretrained_encoder.features, img_size).cuda()
    decoder_chkpt = model.state_dict()
    pretrained_decoder_ckpt = torch.load(args.weights_init_decoder, map_location=device)['model']
    # 1. filter out unnecessary keys
    pretrained_decoder_ckpt = {k: v for k, v in pretrained_decoder_ckpt.items() if k not in ("decoder.last_conv.6.weight", "decoder.last_conv.6.bias")}
