...
def main(args):
    ...
    model = Segmentator(6, pretrained_model.features, img_size).cuda()
    checkpoint = torch.load("binary_segmentation.pth")
    del checkpoint['model']["decoder.last_conv.6.weight"]
    del checkpoint['model']["decoder.last_conv.6.bias"]
    ...
    best_val_miou = 0.0
    writer_2 = SummaryWriter("./tensorboard")
    for epoch in range(100):
        ...
        writer_2.add_scalar('train_loss_mul', train_loss, epoch)
        writer_2.add_scalar('val_loss_mul', val_results[0], epoch)
        writer_2.add_scalar('train_iou_mul', train_iou, epoch)
        writer_2.add_scalar('val_iou_mul', val_results[1], epoch)
...
def train(loader, model, criterion, optimizer, logger):
    ...
    for inputs, labels in loader:
        ...
        total += labels.size(0)
        iou += instance_mIoU(outputs, labels).item()
        count += 1
        loss.backward()
        optimizer.step()
        running_loss += loss.item() * inputs.size(0)
    mean_train_loss = running_loss / total
    mean_train_iou = iou / count
